...
class Payment extends BasicModule {
    ...
    function getUserPaymentHistory($data){
      $tmid = $data['tmid'];
      $usids = (int) $data['usids'];

      if($usids < 0){
          $this->returnedData["data"]["data"] = ($this->db->getConnection())->fetchRowMany('SELECT payment_list.id, payment_list.amount, payment_list.name, payment_status.name as statusName, DATE(payment_list.date_to_pay) AS date_to_pay, user_data.firstname, user_data.lastname FROM payment_list, payment_status, users, user_data WHERE payment_list.id_user = users.id AND user_data.user_id = users.id AND payment_list.id_status = payment_status.id AND payment_list.id_team='.$tmid.' ORDER BY payment_list.id DESC LIMIT 50');
          $this->returnedData["data"]["payed"] = round(($this->db->getConnection())->fetchRow('SELECT SUM(payment_list.amount) AS summary FROM payment_list WHERE payment_list.id_status = 3 AND payment_list.id_team='.$tmid)['summary'],2);
          $this->returnedData["data"]["delayed"] = round(($this->db->getConnection())->fetchRow('SELECT SUM(payment_list.amount) AS summary FROM payment_list WHERE (payment_list.id_status = 1 OR payment_list.id_status = 2) AND payment_list.id_team='.$tmid.' AND NOW() > DATE(payment_list.date_to_pay)')['summary'],2);
          $this->returnedData["data"]["notPayed"] = round(($this->db->getConnection())->fetchRow('SELECT SUM(payment_list.amount) AS summary FROM payment_list WHERE (payment_list.id_status = 1 OR payment_list.id_status = 2) AND payment_list.id_team='.$tmid)['summary'],2);
      }else{
          $this->returnedData["data"]["data"] = ($this->db->getConnection())->fetchRowMany('SELECT payment_list.id, payment_list.amount, payment_list.name, payment_status.name as statusName, DATE(payment_list.date_to_pay) AS date_to_pay, user_data.firstname, user_data.lastname FROM payment_list, payment_status, users, user_data WHERE payment_list.id_user = users.id AND user_data.user_id = users.id AND payment_list.id_status = payment_status.id AND payment_list.id_user='.$usids.' AND payment_list.id_team='.$tmid.' ORDER BY payment_list.id DESC LIMIT 50');
          $this->returnedData["data"]["payed"] = round(($this->db->getConnection())->fetchRow('SELECT SUM(payment_list.amount) AS summary FROM payment_list WHERE payment_list.id_status = 3 AND payment_list.id_user='.$usids.' AND payment_list.id_team='.$tmid)['summary'],2);
          $this->returnedData["data"]["delayed"] = round(($this->db->getConnection())->fetchRow('SELECT SUM(payment_list.amount) AS summary FROM payment_list WHERE (payment_list.id_status = 1 OR payment_list.id_status = 2) AND payment_list.id_team='.$tmid.' AND payment_list.id_user='.$usids.' AND NOW() > DATE(payment_list.date_to_pay)')['summary'],2);
          $this->returnedData["data"]["notPayed"] = round(($this->db->getConnection())->fetchRow('SELECT SUM(payment_list.amount) AS summary FROM payment_list WHERE (payment_list.id_status = 1 OR payment_list.id_status = 2) AND payment_list.id_team='.$tmid.' AND payment_list.id_user='.$usids)['summary'],2);
      }
